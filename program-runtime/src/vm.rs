...
macro_rules! create_vm {
    ($vm:ident, $program:expr, $regions:expr, $accounts_metadata:expr, $invoke_context:expr $(,)?) => {
        ...
        let heap_cost_result = invoke_context.consume_checked($crate::vm::calculate_heap_cost(
            heap_size,
            invoke_context.get_execution_cost().heap_cost,
        ));
        let $vm = heap_cost_result.and_then(|_| {
            let (mut stack, mut heap) = $crate::vm::MEMORY_POOL
                .with_borrow_mut(|pool| (pool.get_stack(stack_size), pool.get_heap(heap_size)));
            let vm = $crate::vm::create_vm(
                $program,
                $regions,
                $accounts_metadata,
                $invoke_context,
...
/// Execute a program with the SBF virtual machine.
